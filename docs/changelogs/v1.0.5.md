...
* **Ungrab stale hotkey grabs before re-grabbing**: When reconnecting, the previous Spud instance may have left passive key grabs on the root window. The new instance would then get an `Access` error from `GrabKey`. The X11 backend now calls `ungrab_key` for all modifier combinations before attempting to grab, preventing the error on reconnect.
